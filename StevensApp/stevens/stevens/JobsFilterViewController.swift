//
//  JobsFilterViewController.swift
//  stevens
//
//  Created by Shuhao Bai on 8/17/19.
//  Copyright © 2019 JqZhang. All rights reserved.
//

import UIKit

// MARK: - IBActions
// https://www.raywenderlich.com/462-storyboards-tutorial-for-ios-part-2
extension CategoryViewController {
    
    @IBAction func cancelJobsFilterViewController(_ segue: UIStoryboardSegue) {
    }
    
    @IBAction func saveJobsFilter(_ segue: UIStoryboardSegue) {
    }
}

class JobsFilterViewController: UIViewController {

    override func viewDidLoad() {
        super.viewDidLoad()

        // Do any additional setup after loading the view.
    }
    

    /*
    // MARK: - Navigation

    // In a storyboard-based application, you will often want to do a little preparation before navigation
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        // Get the new view controller using segue.destination.
        // Pass the selected object to the new view controller.
    }
    */

}
